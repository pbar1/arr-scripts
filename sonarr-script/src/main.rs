...
fn main() -> anyhow::Result<ExitCode> {
    ...
    info!(eventtype = ?cli.eventtype, "handling event");

    let result = match cli.eventtype {
        EventType::Test => handle_test(&cli),
        EventType::Download => handle_download(&cli),
    };

    result.map(|_| ExitCode::SUCCESS)
}

fn handle_test(_cli: &Cli) -> anyhow::Result<()> {
...
fn handle_download(cli: &Cli) -> anyhow::Result<()> {
    ...
    let episodefile_stem = episodefile_path
        .file_stem()
        .context("unable to get episodefile stem")?;
    let subtitle_dir = episodefile_path
        .parent()
        .context("unable to get episodefile_dirname")?
        .join(".subtitles")
        .join(episodefile_stem);
    ...
    let subtitle_streams = get_subtitle_streams(&episodefile_path)?;
    ...
    let live_en = episodefile_path.with_extension("en.srt");
    ...
    if let Some(chs) = get_best_srt_chs(&subtitle_dir) {
        ...
        let live_chs = episodefile_path.with_extension("zh.srt");
        std::fs::copy(&merged, live_chs)?;
    }
    if let Some(cht) = get_best_srt_cht(&subtitle_dir) {
        let merged = merge_srt(&cht, &en)?;
        let live_cht = episodefile_path.with_extension("zh-TW.srt");
